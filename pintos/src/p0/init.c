...
/* Simple kernel monitor: interactive prompt when no kernel args provided. */
static void
kernel_monitor (void)
{
  char buf[128];
  ...
  for (;;)
    {
      ...
      for (;;)
        {
          int c = input_getc ();     /* blocks until a key is pressed */
          if (c == '\r' || c == '\n')
            {
              putchar ('\n');
              buf[pos] = '\0';
      ...
      if (!strcmp (buf, "whoami"))
        {
          printf ("Koen Lin\n");      /* replace with your name if desired */
        }
      else if (!strcmp (buf, "exit"))
        {
          break;                      /* exit monitor, let kernel finish */
        }
      else
        {
